...
import {sortUint32} from '../dist/index.cjs';
...
const seed = [123, 456];
const {randomUint32} = entropy(seed);

let treference = new Uint32Array(N);
fillfn(treference, 0, N, randomUint32);
...
new Benchmark.Suite('Radix sort benchmark')
	...
	.add(
		...
		function () {
			output[this.name] = sortUint32(ta);
		},
		{setup},
	)
	.on('cycle', onCycle)
